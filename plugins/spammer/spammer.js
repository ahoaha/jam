...
class Spammer {
  ...
  async sendPacket (content, type) {
    ...
    if (Array.isArray(content)) return app.jam.client.sendMultiple(content, type)

    if (type === 'remote') app.jam.client.remoteWrite(content)
    else app.jam.client.localWrite(content)
  }
