...
class Client extends EventEmitter {
  ...
  async connect () {
    try {
      ...
      const { host, port } = this.server.app.settings.get('remote')
      ...
      this.server.app.console.showMessage({
  ...
  async remoteWrite (packet) {
    if (this.connectionState === CONNECTION_STATE.CONNECTED) {
      try {
        ...
        this.server.emit('packet', { packet: toPacket, type: 'remote' })
  ...
  async localWrite (packet) {
    if (this.connectionState === CONNECTION_STATE.CONNECTED) {
      try {
        ...
        this.server.emit('packet', { packet: toPacket, type: 'local' })
   ...
   * @param {Number} delay Time to wait between executions
